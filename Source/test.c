...
static void lVKKAT(FILE *fp, int keyLength) {
	int i, j, r;
	BYTE block[4*4];
...
static void lVTKAT(FILE *fp, int keyLength) {
	int i;
	BYTE block[4*4];
	BYTE keyMaterial[320];
	keyInstance keyInst;
...
static void lTKAT(FILE *fp, int keyLength, FILE *in) {
	int i, j;
	unsigned int s;
...
static void lIVKAT (FILE *fp, int keyLength) {
	int i;
	BYTE pt[4*4], ct[4*4];
	BYTE keyMaterial[320];
	keyInstance keyInst;
...
static void makeKATs(const char *vkFile, const char *vtFile, const char *tblFile, const char *ivFile) {
	...
	if (NULL != (fp2 = fopen("table.256", "r"))) {
		...
		printf("Table Known Answer test expects file table.256\n");
		fclose(fp);
		exit(EXIT_FAILURE);
	}
...
static void lECB_MCT(FILE *fp, int keyLength, BYTE direction) {
	int i, j;
...
static void lCBC_MCT(FILE *fp, int keyLength, BYTE direction) {
	int i, j, r, t;
...
static void makeMCTs(const char *ecbEncryptionFile, const char *ecbDecryptionFile,
	...
	lECB_MCT(fp, 128, DIR_ENCRYPT);
	lECB_MCT(fp, 192, DIR_ENCRYPT);
	lECB_MCT(fp, 256, DIR_ENCRYPT);
	...
	lECB_MCT(fp, 128, DIR_DECRYPT);
	lECB_MCT(fp, 192, DIR_DECRYPT);
	lECB_MCT(fp, 256, DIR_DECRYPT);
	...
	lCBC_MCT(fp, 128, DIR_ENCRYPT);
	lCBC_MCT(fp, 192, DIR_ENCRYPT);
	lCBC_MCT(fp, 256, DIR_ENCRYPT);
	...
	lCBC_MCT(fp, 128, DIR_DECRYPT);
	lCBC_MCT(fp, 192, DIR_DECRYPT);
	lCBC_MCT(fp, 256, DIR_DECRYPT);
...
void lSpeed(int keyBits) {
	...
	for (i = 0; i < ITERATIONS; i++) {
		Nr = lKeySetupEnc(rk, cipherKey, keyBits);
	}
	elapsed += clock();
	...
	elapsed = -clock();
	for (i = 0; i < ITERATIONS; i++) {
		Nr = lKeySetupDec(rk, cipherKey, keyBits);
	}
	elapsed += clock();
	sec = (float)elapsed/CLOCKS_PER_SEC;
